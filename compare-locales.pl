...
#       Cédric Corazza <cedric.corazza AT wannadoo.fr>
#         added exclusion of .svn and .hg directories
#
...
sub compareDir
{
    ...
    @entries1 = grep(!(/^(\.|CVS)/ || /~$/ || /^(\.|svn)/ || /^(\.|hg)/), readdir(DIR1));
    closedir(DIR1);

    opendir(DIR2, "$gSourceDir2/$path") ||
        die ("Couldn't list $gSourceDir2/$path");
    %entries2 = map { $_ => 1 } grep(!(/^(\.|CVS)/ || /~$/ || /^(\.|svn)/ || /^(\.|hg)/), readdir(DIR2));
    closedir(DIR2);
